/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package javaapplication67;

public class JavaApplication67 {
    
    public int lalal;
    
    public static int Aleatorio_psein(int numero, int numero2) {
        return 0;
    }
    public static void main(String[] args) {
        //Alumno unAlumno = new Alumno();
        
        //unAlumno.setNombre("Pepito");
        //System.out.println(unAlumno.dameTuNombre());
        
        //Perro unPerro = new Perro();
        
        //System.out.println(unPerro.trotar());
        //System.out.println(unPerro.dameTuNombre());
        Persona NN = new Persona();
        System.out.println("Tu nombre y apellido es: "+ NN.getNombre()+" "+NN.getApellido());
        System.out.println("Estas vestido con "+ NN.dequedoloresturopa());
        
        
        
    }
    
}
